 ...
 * @since 4.0
 */
public class WebSocketInvocableHandlerMethod extends HandlerMethod {
	 ...
	 */
	public WebSocketInvocableHandlerMethod( HandlerMethod handlerMethod ) {
	 ...
	 */
	public WebSocketInvocableHandlerMethod( Object bean, Method method ) {
	 ...
	 */
	public WebSocketInvocableHandlerMethod( Object bean, String methodName, Class<?>... parameterTypes ) throws NoSuchMethodException {
	 ...
	 */
	public final Object invokeWithArguments( ObjectMapper objectMapper, Map<String, Object> dataMap, Object... arguments ) throws Exception {
	
		Object[] args = getMethodArgumentValues( objectMapper, dataMap, arguments );
	 ...
	 * @param session
	 */
	private Object[] getMethodArgumentValues( ObjectMapper objectMapper, Map<String, Object> dataMap, Object... arguments ) throws Exception {
	
		MethodParameter[] parameters = getMethodParameters( );
		String[] parametersName = parameterNameDiscoverer.getParameterNames( getMethod( ) );
		...
		for ( int i = 0; i < parameters.length; i++ ) {
			...
			if ( argumentResolvers.supportsParameter( parameter ) ) {
				try {
					args[i] = argumentResolvers.resolveArgument( parameter, null );
					continue;
				} catch ( Exception ex ) {
					if ( logger.isTraceEnabled( ) ) {
	 ...
	 */
	private Object resolveProvidedArgument( ObjectMapper objectMapper, MethodParameter parameter, String parameterName, Map<String, Object> dataMap, Object... args ) throws JsonParseException,
			JsonMappingException, IOException {
	
		if ( dataMap != null ) {
			if ( dataMap.keySet( ).contains( parameterName ) ) {
				Object object = dataMap.get( parameterName );
				if ( !parameter.getParameterType( ).equals( object.getClass( ) ) ) {
					object = objectMapper.convertValue( object, parameter.getParameterType( ) );
				}
				return object;
			}
		}
		for ( int i = 0; i < args.length; i++ ) {
			Object providedArg = args[i];
